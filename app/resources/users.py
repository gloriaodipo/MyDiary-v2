...
class SignupResource(Resource):
    ...
    def post(self):
        ...
        if not (re.match(username_format, username)):
            ...
            return {'message': 'Invalid email. Ensure email is of the form example@mail.com'}, 200
        if len(username) < 4:
            return {'message' : 'Username should be atleast 4 characters'}, 200
        if len(password) < 8:
            return {'message' : 'Password should be atleast 8 characters'}, 200
